...
public class InquiryReplyDaoJdbcImpl implements InquiryReplyDao {
	...
	public int insertOne(InquiryReplyDTO inquiryreplydto, int maxId) {
		int result = jdbc.update(
				"insert into inquiry_reply (id," + " inquiry_id," + " title," + " content)" + " value(?,?,?,?)",
				inquiryreplydto.getId(), maxId, "返信なし", "返信なし");

		return result;
	}

	public int replyUpdateOne(HttpSession session, InquiryReplyDTO inquiryreplydto) {
		int result = jdbc.update("update inquiry_reply set title = ?,content = ? where inquiry_id = ?",
				session.getAttribute("title"), session.getAttribute("content"), session.getAttribute("id"));
		return result;
	}
